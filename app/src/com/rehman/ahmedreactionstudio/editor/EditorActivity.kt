...
class EditorActivity : Activity(), StageView.Host, RadialMenus.Host {
    ...
    // Standalone torch control — works whether or not a live camera preview
    // is running. This is the same TorchController LiveCamera uses
    // internally, but it must not be gated behind liveCam being non-null:
    // CameraManager.setTorchMode() needs no open camera session at all.
    private val standaloneTorch by lazy { com.rehman.ahmedreactionstudio.camera.TorchController(this) }
    ...
    // Mic gain set from the Audio wheel; survives across recorder instances
    // since CompositionRecorder is recreated for every take.
    private var micGainPref: Float = 1f
    override fun micGain(): Float = micGainPref
    override fun setMicGain(g: Float) {
        micGainPref = g.coerceIn(0f, 3f)
    ...
    private fun releaseStandaloneTorchIfIdle() {
        // don't kill the LED mid-recording if a composite recording is
        // currently using it via liveCam's own TorchController
        if (liveCam == null) standaloneTorch.releaseAll()
    }
    ...
    /**
     * "View" after an export/recording: open the containing folder in
     * whatever file manager the device has, falling back to opening the
     * file itself if that isn't possible.
     *
     * Honest limitation: Android has no universal "show this exact folder"
     * intent contract — support for a directory-typed ACTION_VIEW varies by
     * OEM/file-manager, unlike opening a file, which is always reliable via
     * the MediaStore content:// URI MediaSave already gives us. This tries
     * the folder first (best-effort, several attempts) and only falls back
     * to the file itself if every attempt fails — never a raw file:// URI
     * handed to another app, which would crash under this app's targetSdk
     * (FileUriExposedException).
     */
    private fun openInFileManager(saved: MediaSave.Saved, mime: String) {
        val folderDocUri = try {
            // MediaSave always publishes into the public Movies/<ALBUM>
            // folder on the primary external volume when publiclyVisible —
            // this is the standard SAF document URI for that directory on
            // the vast majority of devices (primary internal storage).
            if (saved.publiclyVisible)
                Uri.parse("content://com.android.externalstorage.documents/document/" +
                    Uri.encode("primary:Movies/${MediaSave.ALBUM}", ":/"))
            else null
        } catch (_: Exception) { null }

        if (folderDocUri != null) {
            for (mimeGuess in arrayOf("vnd.android.document/directory", "resource/folder")) {
                try {
                    val i = Intent(Intent.ACTION_VIEW).apply {
                        setDataAndType(folderDocUri, mimeGuess)
                        addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION or Intent.FLAG_ACTIVITY_NEW_TASK)
                    }
                    if (i.resolveActivity(packageManager) != null) { startActivity(i); return }
                } catch (_: Exception) { /* try the next guess */ }
            }
        }
        // Every folder-open attempt failed (or this build saved outside the
        // public album) — open the file itself, which always works.
        viewRecording(saved.uri, saved.path, mime)
    }
     ...
     * view a real but UNATTACHED object, so `sheetReady()` (isInitialized
     * checks) is true while nothing is actually on screen. `sheetAttached()`
     * is the honest test. The fallbacks below keep every wheel petal working
     * until the sheets are restored (P0-5/P1-2/P1-3) — then they auto-heal,
     * because the normal path runs again once the sheet is attached.
     */
    ...
    override fun isTorchOn(l: Layer): Boolean =
        if (l.id == liveCamLayerId && liveCam != null) liveCam?.torch == true
        else standaloneTorch.isTorchOn(false) || standaloneTorch.isTorchOn(true)

    override fun hasTorch(l: Layer): Boolean =
        if (l.id == liveCamLayerId && liveCam != null) liveCam?.hasFlashUnit == true
        else standaloneTorch.hasFlash(front = false) || standaloneTorch.hasFlash(front = true)

    override fun hasFrontTorch(): Boolean =
        liveCam?.frontHasFlash ?: standaloneTorch.hasFlash(front = true)
    override fun hasBackTorch(): Boolean =
        liveCam?.backHasFlash ?: standaloneTorch.hasFlash(front = false)
    override fun isFrontTorchOn(): Boolean =
        if (liveCam != null) liveCam?.isTorchOnForFront() == true else standaloneTorch.isTorchOn(true)
    override fun isBackTorchOn(): Boolean =
        if (liveCam != null) liveCam?.isTorchOnForBack() == true else standaloneTorch.isTorchOn(false)
    override fun isBothTorchOn(): Boolean =
        if (liveCam != null) liveCam?.bothTorchesFullyOn() == true
        else standaloneTorch.isTorchOn(true) && standaloneTorch.isTorchOn(false)
    ...
    override fun toggleTorch(l: Layer) {
        ...
        // no matching live camera layer running — fall back to the rear LED
        // directly instead of refusing outright
        toggleBackTorch()
    }
    override fun toggleFrontTorch() {
        ...
        if (cam != null) {
            if (!cam.hasFlashForFront()) {
                UI.toast(this, "Front camera has no LED — use the screen light"); return
            }
            if (!cam.toggleFrontTorch() && !cam.isTorchOnForFront()) {
                UI.toast(this, cam.torchLastError().takeIf { it.isNotBlank() } ?: "Front flash unavailable")
                return
            }
            UI.toast(this, if (cam.isTorchOnForFront()) "Front flash on" else "Front flash off")
            refreshAll(); return
        }
        // no live camera preview running — drive the LED directly
        standaloneTorch.start()
        if (!standaloneTorch.hasFlash(front = true)) {
            UI.toast(this, "Front camera has no LED — use the screen light"); return
        }
        val on = !standaloneTorch.isTorchOn(true)
        if (!standaloneTorch.setTorch(true, on)) {
            UI.toast(this, standaloneTorch.failureText().takeIf { it.isNotBlank() } ?: "Front flash unavailable")
            return
        }
        UI.toast(this, if (on) "Front flash on" else "Front flash off")
        refreshAll()
    }
    override fun toggleBackTorch() {
        ...
        if (cam != null) {
            if (!cam.hasFlashForBack()) {
                UI.toast(this, "This device has no rear flash"); return
            }
            if (!cam.toggleBackTorch() && !cam.isTorchOnForBack()) {
                UI.toast(this, cam.torchLastError().takeIf { it.isNotBlank() } ?: "Rear flash unavailable")
                return
            }
        ...
        standaloneTorch.start()
        if (!standaloneTorch.hasFlash(front = false)) {
            UI.toast(this, "This device has no rear flash"); return
        }
        val on = !standaloneTorch.isTorchOn(false)
        if (!standaloneTorch.setTorch(false, on)) {
            UI.toast(this, standaloneTorch.failureText().takeIf { it.isNotBlank() } ?: "Rear flash unavailable")
            return
        }
        UI.toast(this, if (on) "Rear flash on (LED)" else "Rear flash off")
        refreshAll()
    }
    override fun toggleBothTorch() {
        ...
        if (cam != null) {
            if (!cam.hasFlashForFront() && !cam.hasFlashForBack()) {
                UI.toast(this, "This device has no camera flash — use the screen light"); return
            }
            val turnOn = !cam.bothTorchesFullyOn()
            if (turnOn && !cam.setBothTorches(true)) {
                UI.toast(this, "No camera flash is available"); return
            }
        ...
        standaloneTorch.start()
        val hasFront = standaloneTorch.hasFlash(front = true)
        val hasBack = standaloneTorch.hasFlash(front = false)
        if (!hasFront && !hasBack) {
            UI.toast(this, "This device has no camera flash — use the screen light"); return
        }
        val turnOn = !(standaloneTorch.isTorchOn(true) && standaloneTorch.isTorchOn(false))
        var ok = true
        if (hasFront) ok = standaloneTorch.setTorch(true, turnOn) && ok
        if (hasBack) ok = standaloneTorch.setTorch(false, turnOn) && ok
        UI.toast(this, if (ok) (if (turnOn) "Both flashes on" else "Both flashes off") else "No camera flash is available")
        refreshAll()
    }
    ...
    // ---------------- Play/Stop radial wheel ----------------

    override fun isRecordingComposite(): Boolean = recording

    override fun canRecordComposite(): Boolean {
        val p = proj ?: return false
        return p.layers.any { it.isLive() } && p.layers.any { it.isClip() }
    }

    override fun toggleCompositeRecording() = recordButtonTap()
