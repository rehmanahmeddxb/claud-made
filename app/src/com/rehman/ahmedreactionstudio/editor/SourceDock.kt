...
class SourceDock(
    ...
    // Resolves a layer's relPath to an absolute file path for thumbnailing.
    // Kept as an injected lambda (not a ProjectStore reference) so this view
    // class doesn't need to know how projects are laid out on disk.
    ...
    companion object {
        // Process-lifetime thumbnail cache. Keyed on path + file length so a
        // file that gets re-recorded/replaced under the same name never
        // reuses a stale (or black) cached bitmap forever.
        private val thumbCache = java.util.concurrent.ConcurrentHashMap<String, android.graphics.Bitmap>()
        private val thumbExecutor = java.util.concurrent.Executors.newFixedThreadPool(2)
        private fun cacheKey(path: String, len: Long): String = "$path:$len"
    }
    ...
    private fun buildRow(l: Layer): LinearLayout {
        ...
        // Static type glyph for TEXT and live CAMERA (nothing on disk to
        // thumbnail for either). Everything file-backed (VIDEO/IMAGE/SCREEN,
        // and a non-live CAMERA take) gets an actual decoded preview,
        // loaded async and cached — never blocking row build.
        val typeIc = ImageView(act)
        val tlp = LinearLayout.LayoutParams(UI.dp(act, 28), UI.dp(act, 28))
        ...
        bg2.cornerRadius = UI.dpf(act, 6f)
        bg2.setColor(Color.argb(160, 8, 9, 13))
        typeIc.background = bg2
        typeIc.setImageDrawable(Ic.get(act, Ic.typeIcon(l.type),
            if (l.visible) UI.ACCENT2 else Color.argb(120, 255, 255, 255)))
        val absPath = if (!l.isLive() && !l.isText()) mediaAbsPath(l) else null
        if (absPath != null) loadThumb(typeIc, absPath, l)
        row.addView(typeIc)
    ...
    /**
     * Decode + cache a real thumbnail for a file-backed source, off the main
     * thread. Two things this deliberately gets right (see docs/BLUEPRINT
     * §8/§9 — the audited "always black thumbnail" bug):
     *  - never decodes a video frame at ms=0: freshly recorded/composited
     *    video very often has a black/garbage first frame while the encoder
     *    warms up, so we grab a frame a few hundred ms in and only fall back
     *    to 0 if that genuinely fails.
     *  - cache key includes file length, not just the path, so re-recording
     *    over the same filename can never permanently reuse a stale (or
     *    black) cached bitmap.
     * `img.tag` is used as the row-identity guard: if this row gets rebuilt
     * (recycled into a different layer) before the decode finishes, the
     * stale callback is a no-op instead of painting the wrong thumbnail.
     */
    private fun loadThumb(img: ImageView, path: String, l: Layer) {
        val file = java.io.File(path)
        if (!file.exists()) return
        val len = file.length()
        val key = cacheKey(path, len)
        img.tag = key
        thumbCache[key]?.let { img.setImageBitmap(it); return }

        thumbExecutor.execute {
            val bmp = try {
                if (l.isVideoLike()) {
                    MediaKit.videoFrame(path, 300_000L, UI.dp(act, 96), true)
                        ?: MediaKit.videoFrame(path, 0L, UI.dp(act, 96), true)
                } else {
                    MediaKit.image(path, UI.dp(act, 96))
                }
            } catch (_: Throwable) { null } ?: return@execute
            thumbCache[key] = bmp
            act.runOnUiThread {
                if (img.tag == key) img.setImageBitmap(bmp)
            }
        }
    }
