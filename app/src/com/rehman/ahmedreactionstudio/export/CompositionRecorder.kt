...
class CompositionRecorder(
     ...
     *
     * UI Plan2 T-14 — DECISION (2026-09-05), refined: `masterGain` stays
     * **internal** — `Exporter` has no equivalent clip-audio gain stage, so
     * surfacing a master fader would change a recording but not a re-export
     * of the same project (the "control that only affects one path" Rule 4
     * forbids). `micGain` does NOT have that problem: the microphone is only
     * ever captured live, here, during a composite recording — there is no
     * separate export path that also touches raw mic input to fall out of
     * sync with. It is safe to expose and is now wired to the Audio wheel's
     * Mic petal (EditorActivity.setMicGain).
     *
     * Per-source level in the Audio sheet (V07) already covers the real need
     * for clip volume. If a clip-audio master fader is ever wanted, the
     * correct order is still: add the same gain stage to `Exporter`, prove
     * both paths null-test identical, and only then add the UI — do not
     * shortcut that by exposing `masterGain`.
     */
    @Volatile var micGain: Float = 1f
        set(v) { field = v.coerceIn(0f, 3f) }
                ...
                while (!stopRequested) {
                    ...
                    if (micGain != 1f) {
                        for (i in out.indices) {
                            val g = (out[i] * micGain).toInt()
                            out[i] = g.coerceIn(Short.MIN_VALUE.toInt(), Short.MAX_VALUE.toInt()).toShort()
                        }
                    }
